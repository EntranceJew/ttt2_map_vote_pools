...
function SKIN:PaintTextAreaTTT2(panel, w, h)
	local settingsBox = util.GetChangedColor(vskin.GetBackgroundColor(), 150)
	local settingsText = util.GetDefaultColor(util.GetChangedColor(vskin.GetBackgroundColor(), 150))

	if not panel:IsEnabled() then
		colorBox = ColorAlpha(settingsBox, 100)
		colorText = ColorAlpha(settingsText, 100)
	end
